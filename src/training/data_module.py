...
def get_data_2020(root_dir: str, labeled: bool = True) -> dict:
    ...
    :return: path dict
    """
    assert root_dir.split('/')[-1].startswith(
        'MICCAI_BraTS2020'
    ), 'Invalid directory!'
...
class BraTSDataModule(LightningDataModule):
    ...
    def __init__(
        ...
        log(
            f'Based on the directory, assuming <{self.dataset}> dataset.',
            verbosity=1,
        )
        ...
        self.training_data = [
            self.data[idx] for idx in self.folds[self.fold_index][0]
        ]
        self.validation_data = [
            self.data[idx] for idx in self.folds[self.fold_index][1]
        ]
    ...
    def setup(self, stage=None):
        ...
        if stage == 'fit' or stage is None:
            ...
            self.training_set = Dataset(
                data=self.training_data, transform=self.training_transform
            )
            self.validation_set = Dataset(
                data=self.validation_data, transform=self.validation_transform
            )
        ...
        if stage == 'test' or stage is None:
            ...
            self.test_set = Dataset(
                data=self.test_data, transform=self.test_transform
            )
...
if __name__ == '__main__':
    ...
    brats = PooyaModule(root_dir=data_dir, batch_size=1, seed=616, cache_num=1)
    brats.setup('fit')
