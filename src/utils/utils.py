...
def get_tuning_par(
    ...
    assert (
        tensor_net_type in TENSOR_NET_TYPES
    ...
    if tensor_net_type in ['tt', 'tensor-train', 'train']:
        ...
        solutions = solve(
            max_params / (r * (in_channels + 3 * (3 * r) + out_channels))
            - compression,
            r,
        )
...
def get_network_size(
    ...
    if tensor_net_type in ['cp', 'cpd', 'canonical']:
        ...
        return int(tuning_param) * (
            in_channels + out_channels + 3 * kernel_size
        )
    ...
    ##########
        ...
        """
        in_param = (
            round(in_channels / tuning_param)
            if round(in_channels / tuning_param) > 0
            else 1
        )
        out_param = (
            round(out_channels / tuning_param)
            if round(out_channels / tuning_param) > 0
            else 1
        )
...
if __name__ == '__main__':
    ...
    for comp in compression_rates:
        ...
        tuning_tucker.append(
            get_tuning_par(
                compression=comp,
                tensor_net_type='tucker',
                in_channels=in_channels,
                out_channels=out_channels,
                kernel_size=kernel_size,
            )
    ...
    df['compression_error'] = (
        np.absolute(df.actual_compression - df.Compression) / df.Compression
    )
    ...
    sns.set_theme(
        context='paper', font_scale=1.3, style='white', palette=colors
    )
    sns.lineplot(
        data=df,
        x='Compression',
        y='actual_compression',
        hue='Format',
        style='Format',
        markers=False,
        markersize=8,
    ...
    plt.tight_layout()
    plt.savefig(
        join(vis_dir, 'param_vs_compression.pdf'),
        bbox_inches='tight',
        pad_inches=0,
    )
    plt.show()
