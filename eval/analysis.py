...
def add_macs(row: pd.Series, macs_params: pd.DataFrame) -> int:
    # Parameters
    type = row['type']
    ...
    # Find
    expr = f"type == '{type}' & compression == {compression} & widths == '{widths}' & kernel_size == {kernel}"

    # print(expr, '->',)
    # print(int(macs_params.query(expr)['macs']))

    return int(macs_params.query(expr)['macs'])


def add_params(row: pd.Series, macs_params: pd.DataFrame) -> int:
    # Parameters
    type = row['type']
    compression = row['compression']
    widths = row['widths']
    ...
    return int(macs_params.query(expr)['params'])
...
def build_metrics_dataframe():
    ...
    macs_params = pd.read_csv('results/df_macs_params.csv', index_col=0)
    ...
    metrics['macs'] = metrics.progress_apply(lambda row: add_macs(row, macs_params=macs_params), axis=1)
    metrics['params'] = metrics.progress_apply(lambda row: add_params(row, macs_params=macs_params), axis=1)

    metrics.rename(columns={
        'type': 'Format',
        'compression': 'Compression',
        'params': 'Parameters',
        'macs': 'MACs'}, inplace=True)
    metrics.replace({
        'cp': 'Canonical polyadic',
        'tt': 'Tensor train',
        'tucker': 'Tucker',
        'base': 'Baseline'}, inplace=True)
    metrics.rename(columns={
    ...
    metrics.to_csv("results/df_metrics.csv", index=False)
...
if __name__ == '__main__':
    ...
    # If requested, rebuild data frame
    if rebuild:
        hlp.log('Rebuilding data frame.')
        metrics = build_metrics_dataframe()
    else:
        metrics = pd.read_csv("results/df_metrics.csv")

    # Restrictions
    metrics = metrics.loc[metrics.widths == '(48, 96, 192, 384, 768)']
    ...
    for x_var in ('Compression',):  # , 'Network compression','MACs'):
        ...
        for idx, measure in enumerate(
            ...
            low, up = baseline_mean - z_val * baseline_sd / math.sqrt(
                n), baseline_mean + z_val * baseline_sd / math.sqrt(n)

            ax[row_idx, col_idx].axhline(baseline_mean, xmin=.02, xmax=1.02, linestyle='--', color='#FB7E3A',
                                         label='Baseline')
            ...
            ax[row_idx, col_idx].set_ylabel(measure, fontdict={
                'weight': 'bold'})
            if x_var == 'Compression': ax[row_idx, col_idx].set_xticks([2, 5, 10, 20, 35, 50, 75, 100])
            if row_idx < 2:
                ax[row_idx, col_idx].set_xlabel('')
            else:
                ax[row_idx, col_idx].set_xlabel('Layer compression' if x_var == 'Compression' else x_var, fontdict={
                    'weight': 'bold'})
