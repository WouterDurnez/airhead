...
def build_metrics_dataframe():
    ...
    metrics['macs'] = metrics.progress_apply(
        lambda row: add_macs(row, macs_params=macs_params), axis=1
    )
    metrics['params'] = metrics.progress_apply(
        lambda row: add_params(row, macs_params=macs_params), axis=1
    )
...
if __name__ == '__main__':
    ...
    plt.tight_layout()
    plt.savefig(
        join(vis_dir, 'macs_params.pdf'), bbox_inches='tight', pad_inches=0
    )
    plt.show()
    ...
    for x_var in ('Compression',):  # , 'Network compression','MACs'):
        ...
        for idx, measure in enumerate(
            ...
            baseline_mean = metrics.loc[metrics.Format == 'Baseline'][
                measure
            ].mean()
            baseline_sd = metrics.loc[metrics.Format == 'Baseline'][
                measure
            ].std()
            ...
            sns.lineplot(
                data=metrics.loc[metrics.Format != 'Baseline'],
                x=x_var,
                y=measure,
                err_style='bars',
                err_kws={'capsize': 5},
            ...
            if row_idx == 0 and col_idx == 1:
                legend = plt.legend(
                    title='Format', bbox_to_anchor=(1.05, 1), loc='upper left'
                )
                plt.setp(
