...
def look_one_way(
    data: pd.DataFrame, fixed_var: str, fixed_level: str, metric: str = 'Dice'
):
...
if __name__ == '__main__':
    ...
    metrics_tt = metrics_tensor.loc[
        metrics_tensor.Format == 'Tensor train (v1)'
    ]
    metrics_tt2 = metrics_tensor.loc[
        metrics_tensor.Format == 'Tensor train (v2)'
    ]
    metrics_cp = metrics_tensor.loc[
        metrics_tensor.Format == 'Canonical polyadic'
    ]
    ...
    for format in (
        ...
        for compression in (2, 5, 10, 20, 35, 50, 75, 100):
            ...
            for measure in (
                ...
                b = metrics_tensor.loc[
                    (metrics_tensor.Format == format)
                    & (metrics_tensor.Compression == compression)
                ][measure]
    ...
    table_df = table_df.pivot_table(
        index=['Format', 'Layer compression'],
        columns='Measure',
        values=['U', 'p'],
    )
