...
if __name__ == '__main__':
    ...
    predictions = {type: {} for type in types}
    predictions['baseline'] = np.array(sample['baseline']).take(
        indices=slice, axis=axis
    )
    for type, comp in models:
        predictions[type][comp] = np.array(sample[type][comp]).take(
            indices=slice, axis=axis
        )
    ...
    for row_index, type in enumerate(types, 1):
        for col_index, comp in enumerate(compressions):
            ...
            ax[row_index, 0].set_ylabel(
                pretty_type[type], fontdict={'weight': 'bold'}
            )
            ax[row_index, col_index].imshow(
                img[0, ...], alpha=0.5, cmap='gray'
            )
            ax[row_index, col_index].imshow(
                predictions[type][comp], alpha=alpha, cmap=cmap_mask
            )

            if row_index == 4:
                ax[row_index, col_index].set_xlabel(
                    comp, fontdict={'weight': 'bold'}
                )
    ...
    plt.savefig(
        join(vis_dir, 'prediction_overview.pdf'),
        bbox_inches='tight',
        pad_inches=0,
    )
    plt.show()
    ...
    # Top row, column 5, ground truth
    ax[4].imshow(img[0, ...], alpha=0.5, cmap='gray')
    ...
    plt.legend(
        handles=[
            ...
            for col, lab in zip(
                (colors[0], colors[2], colors[1]), ('ET', 'TC', 'WT')
            )
        ],
    ...
    plt.savefig(
        join(vis_dir, 'dataset_overview.pdf'),
        bbox_inches='tight',
        pad_inches=0,
    )
    plt.show()
